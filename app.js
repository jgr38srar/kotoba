(() => {
    ...
    main.innerHTML = `
      <section class="card">
        ...
        <div class="choice-group">
          ...
          ${radioCard("language", "fi", "フィンランド語", s.language === "fi")}
        </div>
  ...
  function handleAnswer(selected, correct, word, question) {
    ...
    const studyLinks = buildStudyLinksHtml(word, activeSession.language);
  ...
  function mistakeItemHtml(item) {
    ...
    const foreignWord = getForeignWord(item.word, item.language);
    const pinyin = item.language.startsWith("zh") ? getReading(item.word, item.language) : "";
    const studyLinks = buildStudyLinksHtml(item.word, item.language);

    return `<details class="history-item">
      <summary>
        <div class="history-summary">
          <div>
            <strong>${escapeHtml(foreignWord)} <span style="color:var(--muted);font-weight:500">＝ ${escapeHtml(item.word.japanese)}</span></strong>
            ${pinyin ? `<small style="display:block;margin-top:4px">${escapeHtml(pinyin)}</small>` : ""}
            <small style="display:block;margin-top:4px">${escapeHtml(languageLabels[item.language])}・誤答 ${stat.incorrect}回・正解 ${stat.correct}回・タップして追加学習</small>
          </div>
  ...
  function getMistakeWords() {
    ...
    for (const [wordId, stat] of Object.entries(state.wordStats)) {
      ...
      else if (wordId.startsWith("fi")) language = "fi";
    ...
    main.innerHTML = `
      <section class="card">
        ...
        <div class="setting-row">
          ...
          <p>中国語260語、韓国語110語、エスペラント語260語、フィンランド語150語を収録しています。</p>
        </div>
      </section>

      <section class="section notice">
        中国語とエスペラント語はA2〜B1程度、韓国語はA1〜A2程度、フィンランド語はA2程度の日常語彙を中心に収録しています。
      </section>
    `;
  ...
  function getStudyLinks(word, language) {
    const foreignWord = getForeignWord(word, language);
    ...
    const sourceLanguage = getGoogleTranslateLanguage(language);
    ...
    const chatGptPrompt = `次の${languageLabels[language]}の文章を文法的に解説してください。\n\n${example.foreign}`;
    const chatGptUrl = `https://chatgpt.com/?q=${encodeURIComponent(chatGptPrompt)}`;

    return { googleSearchUrl, googleTranslateUrl, chatGptUrl };
  }

  function buildStudyLinksHtml(word, language) {
    const links = getStudyLinks(word, language);
    const linkStyle = "display:flex;align-items:center;justify-content:center;text-decoration:none;text-align:center";

    return `<div class="button-stack" style="margin:12px 0 10px">
      <a class="secondary-button full"
         style="${linkStyle}"
         href="${escapeAttr(links.googleSearchUrl)}"
         target="_blank"
      ...
      <a class="secondary-button full"
         style="${linkStyle}"
         href="${escapeAttr(links.googleTranslateUrl)}"
         target="_blank"
         rel="noopener noreferrer external">
        例文をGoogle翻訳で確認
      </a>
      <a class="secondary-button full"
         style="${linkStyle}"
         href="${escapeAttr(links.chatGptUrl)}"
         target="_blank"
         rel="noopener noreferrer external">
        例文をChatGPTで解説
      </a>
