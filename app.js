  ...
  function handleAnswer(selected, correct, word, question) {
      ...
      <div class="feedback ${isCorrect ? "success" : "error"}">
        ...
        ${!isCorrect ? `
          <div class="button-stack" style="margin:12px 0">
            <button class="secondary-button full" id="googleSearchButton">
              この単語をGoogleで検索
            </button>
            <button class="secondary-button full" id="googleTranslateButton">
              例文をGoogle翻訳で確認
            </button>
          </div>
        ` : ""}
        <button class="primary-button full" id="nextQuestion">
    ...
    if (!isCorrect) {
      document.getElementById("googleSearchButton").addEventListener("click", () => {
        const query = `${foreignWord} ${word.japanese}`;
        openExternalUrl(`https://www.google.com/search?q=${encodeURIComponent(query)}`);
      });

      document.getElementById("googleTranslateButton").addEventListener("click", () => {
        const sourceLanguage = getGoogleTranslateLanguage(activeSession.language);
        const translateUrl = `https://translate.google.com/?sl=${encodeURIComponent(sourceLanguage)}&tl=ja&text=${encodeURIComponent(example.foreign)}&op=translate`;
        openExternalUrl(translateUrl);
      });
    }
  ...
  function renderSettings() {
    main.innerHTML = `
      ...
      <section class="section notice">
        収録語彙は中国語60語、韓国語60語、エスペラント語60語です。日常的なA1〜A2程度を目安にした初版です。
      </section>
    `;
